...
Answer here:
because as the alpha value increases, the colors overlap each other, and we will not be able determine the values we want 
...
ggplot(data = plastic_waste, 
  ...
  geom_violin()
```
Answer: box plots provide a coarser summary of the data, provide more of a summary of a distribution can also be seen as an advantage in certain cases , also the point in the graph or mapping its most clearly than violin plots 
...
```
Answer: Europe (positive) relationship, Africa is the nost (mismanaged_plastic waste per cap)
...
plastic_waste %>% 
  ...
  ggplot(aes(x = coastal_pop_prop, y = plastic_waste_per_cap, color = continent)) + 
    geom_point() +
    geom_smooth(color = "black",method = "gln")+
    scale_color_viridis_d() +
    labs(x = "Coastal population proportion (Coastal / total population)", 
         y = "Plastic waste per capita ", 
         color = "Continent",
    ...
    theme_minimal()
```
Answer : gemo_smooth (aids the eye in seeing patterns in the presence of over plotting also its help us to do see the curve of the data and analysis it via a different way) the coastal cites in Asia and north America is have the most plastic waste per capita and the lowest population 
